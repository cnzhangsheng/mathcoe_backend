...
CREATE TABLE users (
    ...
    difficulty_level INTEGER NOT NULL DEFAULT 1,        -- 难度等级 1-6
...
COMMENT ON COLUMN users.difficulty_level IS '难度等级 1-6';
...
CREATE TABLE questions (
    ...
    difficulty_level INTEGER,                            -- 级别 1-6
    source_year INTEGER,
    tags JSONB DEFAULT '[]',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
...
COMMENT ON COLUMN questions.difficulty_level IS '题目级别 1-6（对应袋鼠数学等级）';
...
COMMENT ON COLUMN exam_papers.difficulty_level IS '难度等级 1-6';
